...
public class ProcessLotofacil extends AbstractProcess {
    ...
    private static final Loteria LOTOFACIL = Loteria.LOTOFACIL;
    private static final int THRESHOLD = 10;

    public ProcessLotofacil(File dataDir) {
        super(dataDir);
    }
    ...
    public void run() {
        ...
        if (sorteios == null || sorteios.length == 0) {
            print("{0}: Arquivo CSV esta vazio. ERRO: Processo abortado.", LOTOFACIL.nome);
            return;
        }

        // efetua a geracao dos jogos de acordo com as combinacoes de jogos da loteria:
        int[][] jogos = Stats.geraCombinacoes(LOTOFACIL.qtdDezenas, LOTOFACIL.qtdBolas);
        int qtdJogos = jogos.length;

        // Efetua a execucao de cada processo de analise em sequencia (chain) para coleta de dados:
        AbstractCompute[] computeChain = this.initComputeChain(LOTOFACIL, jogos, sorteios, THRESHOLD);
        ...
        print("\n{0}: Finalizado o processamento de  {1}  combinacoes de jogos.", LOTOFACIL.nome, qtdJogos);
